...
export default function NosPlats() {
  ...
  return (
    <section className="section section--dark">
      ...
      <div className="cards">
        {plats.map((p, idx) => (
          <article className="card" key={p._id} data-reveal style={{ '--reveal-delay': `${idx * 0.08}s` }}>
            ...
            <div className="card-body">
              <h3 className="card-title" onClick={() => navigate(`/produit/${p._id}`)}>{p.name}</h3>
      ...
      <div className="steps-cta" style={{ marginTop: '2.5rem' }}>
        <Link to="/produits" className="btn-outline">Voir tout le menu</Link>
      </div>
    </section>
  );
}
